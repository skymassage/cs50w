...
document.addEventListener('DOMContentLoaded', function() {
    /* The class name in HTML is "card-footer comment postId_{{ post.id }}".
       In JS, we can use ".card-footer.comment" to select all these tags that don't contain "postId_{{ post.id }}"
       and replace the spaces of "card footer comment" with "." */
    document.querySelectorAll('.card-footer.comment').forEach(comment_div => {
    ...
    /* We need to declare two variables for each <a class="comment_link">. 
       ...
       Note that we should declare the variables here, because executing JavaScript is before the */
                    ...
                    convert_to_html.innerHTML = `
                        <div class="d-flex justify-content-between">
                            <div class="text-start">
                                <a class="text-dark" href="{% url 'profile' user=${comment.author.username} %}">${comment.author}</a>
                            </div>  
